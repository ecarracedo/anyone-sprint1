...
def get_public_holidays(public_holidays_url: str, year: str) -> DataFrame:
    ...
    try:
        ...
        response.raise_for_status()  # Lanza un error si la respuesta no fue exitosa
    except requests.RequestException as e:
        raise SystemExit(f"Failed to fetch public holidays: {e}")

    # Pasammos la respuest JSON a la variable data
    data = json_read.loads(response.text)
